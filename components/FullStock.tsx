    ...
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="flex flex-wrap gap-6">
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">Adults</p>
                <div className="flex items-center gap-3">
                  <button onClick={() => setFamilyAdults(Math.max(1, familyAdults-1))} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">-</button>
                  <span className="text-xl font-bold w-4 text-center">{familyAdults}</span>
                  <button onClick={() => setFamilyAdults(familyAdults+1)} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">+</button>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">Children</p>
                <div className="flex items-center gap-3">
                  <button onClick={() => setFamilyKids(Math.max(0, familyKids-1))} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">-</button>
                  <span className="text-xl font-bold w-4 text-center">{familyKids}</span>
                  <button onClick={() => setFamilyKids(familyKids+1)} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">+</button>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">Current Occasion</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          {/* Casting Object.entries to ensure category and items are typed correctly instead of unknown */}
          {(Object.entries(categorizedInventory) as [Category, InventoryItem[]][]).map(([category, items]) => (
            <Card key={category} className="p-0 overflow-hidden border-slate-100 shadow-xl">
              <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h4 className="font-extrabold text-slate-800 flex items-center gap-3">
                  <span className="w-2 h-6 bg-emerald-500 rounded-full"></span>
                  {category}
                </h4>
                {/* Fix: Property 'length' does not exist on type 'unknown' */}
                <Badge className="bg-white text-slate-500 border border-slate-200">{items.length} items</Badge>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-white border-b border-slate-50">
                      <th className="px-6 py-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">Item</th>
                      <th className="px-6 py-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest text-center">Current</th>
                        ...
                        <tr key={item.id} className="group hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center justify-center gap-3">
                              <button onClick={() => onUpdateQuantity(item.id, Math.max(0, item.quantity - 1))} className="w-6 h-6 rounded bg-slate-100 text-slate-400 hover:bg-slate-200">-</button>
                              <span className="text-sm font-bold w-12 text-center text-slate-800">{item.quantity} {item.unit}</span>
                              <button onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className="w-6 h-6 rounded bg-slate-100 text-slate-400 hover:bg-slate-200">+</button>
                            </div>
                          </td>
                          <td className="px-6 py-5">
                            <div className="flex items-center justify-center gap-3">
                              <button onClick={() => onUpdateTarget(item.id, Math.max(0, item.targetQuantity - 1))} className="w-6 h-6 rounded bg-emerald-50 text-emerald-400 hover:bg-emerald-100">-</button>
                              <span className="text-sm font-bold w-12 text-center text-emerald-700">{scaledTarget}</span>
                              <button onClick={() => onUpdateTarget(item.id, item.targetQuantity + 1)} className="w-6 h-6 rounded bg-emerald-50 text-emerald-400 hover:bg-emerald-100">+</button>
                            </div>
                          </td>
                          <td className="px-6 py-5">
                            <div className="w-full flex items-center gap-3">
                              <div className="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                <div 
                                  className={`h-full transition-all duration-700 ${isLow ? 'bg-amber-400' : 'bg-emerald-500'}`}
                                  style={{ width: `${Math.min(100, (item.quantity / scaledTarget) * 100)}%` }}
                                />
                              </div>
                              {isLow && <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></div>}
                            </div>
                          </td>
                        </tr>
                      );
        ...
        <div className="lg:col-span-4 space-y-8 sticky top-28">
              ...
              {orderList.length > 0 ? orderList.map(item => (
                <div key={item.id} className="bg-white/10 p-3 rounded-2xl flex justify-between items-center backdrop-blur-sm border border-white/5">
                  <div>
                    ...
                    <p className="text-[10px] font-medium text-emerald-200 uppercase">{item.category}</p>
                  </div>
                ...
                <div className="text-center py-6 opacity-60 italic text-sm">Everything is perfectly stocked!</div>
              )}
            </div>

            <Button 
              variant="primary" 
              className="w-full bg-white text-emerald-700 hover:bg-emerald-50 font-black py-4 rounded-2xl shadow-lg"
          ...
          {marts.length > 0 && (
            <div className="space-y-4 animate-in slide-in-from-right-4 duration-500">
              <h5 className="font-black text-slate-800 text-sm px-2 uppercase tracking-widest">Connect to Partner Marts</h5>
              {marts.map((mart, i) => (
                <button 
                  key={i} 
                  onClick={() => handleSendToMart(mart)}
                  className="w-full text-left bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group flex items-center justify-between"
                >
                  <div>
                    <p className="font-bold text-slate-800">{mart.name}</p>
                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-tighter">Instant Delivery Available</p>
                  </div>
