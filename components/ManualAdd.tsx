    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 font-medium italic">Manually log your inventory arrivals.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3 space-y-2">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-2">Manifest Categories</p>
          ...
          <Card className="p-8 bg-white border-slate-100 shadow-xl rounded-[2.5rem]">
            <div className="flex flex-col md:flex-row gap-6 items-end mb-10">
              <div className="flex-1 space-y-2 w-full">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Custom Entry</label>
                <input 
                  type="text"
                  placeholder="Scan or type item name..."
                  value={customName}
                  onChange={(e) => setCustomName(e.target.value)}
                  className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500/30 transition-all"
                />
              </div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 ml-1">Rapid Deployment Menu</p>
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
              {COMMON_ITEMS[activeCategory]?.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleQuickAdd(item)}
                  className="p-4 bg-white border border-slate-100 rounded-2xl hover:border-emerald-500/40 hover:bg-emerald-50/30 transition-all group text-left"
                >
                  <p className="font-bold text-slate-800 group-hover:text-emerald-700">{item.name}</p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">{item.unit}</p>
                </button>
              ))}
            </div>
          </Card>
